...
- 🗂️ **すぐ使える8テンプレート** —— ミニマル · 同人ブース · 印刷所 zine · カフェ · 手作り市 · ピクセル · ブティック · ナイトネオン。各テンプレートに固有のフォント・配色・個性。1つ選んで調整、または空白から。
- 🪙 **任意の通貨** —— コードを選ぶか、自分の記号（`NT$`、`¥` …）を直接入力。エディタはそのまま表示。
- ✶ **枠なしベクターステッカー** —— クリーンなソリッド塗りのマーク集（絵文字なし）、キャンバス上で拡縮・回転可。
...
- **テンプレートから開始** → すぐ使える8スタイル（または空白）から、自分用に調整。
