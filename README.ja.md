...
- 🎨 **テーマ** — `custom`(カラフルなデフォルト)と `thermal`。`custom` は色の変更・フォントの選択・ステッカーの追加ができ、`thermal` は等幅フォントで、埋め込んだ画像を自動で白黒に変換します。
...
| `@receipt-engine/themes` | 組み込みテーマ（`custom` / `thermal`）+ `getTheme` / `mergeTheme`。 |
...
オプション:`--theme custom|thermal`、`--format svg|html|png`、`--out <path>`、
